...
$(function () { //// jQB /////////////////////////////
    ...
    $("#gnb a,.bnav a").click(function (e) {
        ...
        // 만약 #gnb a 이면 1을 더함(배너메뉴가 없으므로! )
        let isGNB = $(this).parent().parent().is("ul#gnb");
        // parent() 를 두번 쓴 것은 li 위에 ul 인지 ol인지로 올라가서
        // is() 메서드로 이것이 ul#gnb 인것을 확인한ㄷ!
        // 이것이 맞으면 true 가 리턴된다!
    ...
    // 드래그 설정
    sld.draggable({
        axis: "x" // x축고정 (위아래로 드래그되는것 막기)
    });
    // drag and drop 응용에 관한 것은 하단경로 포스트잇 활동 참고
    // jQuery실전연습 > 04.utility > 05.DragGallery

    ///////////////////////////////////////////////////////
    // 배너 슬라이드 기능 구현
    // 대상: .slide
    // 기능: 슬라이드를 드래그 하여 슬라이드를 넘긴다
    // 이벤트: 드래그가 끝난 시점에 대한 이벤트 처리
    // 1) dragstop : 드래그가 멈췄을 때
    // 2) touchend : 터치 스크린에서 터치가 끝났을 때
    // { 드래그 기능 구현하기 }
    // 1. 드래그가 끝난 시점에 각 배너의 width 값 10%를 기준해서
    //    left 값이 -100%이므로
    // 2. -110%보다 작으면 슬라이드를 왼쪽으로 애니메이션 이동 
    // 3. -90%보다 크면 슬라이드를 오른쪽으로 애니메이션 이동 
    // 4. -110%와 -90% 범위 사이면 원래위치인 -100%로 복귀
    // (유의사항) 실제 left 값을 구하면 px 단위이므로 
    //           기존값을 화면크기의 px 로 변환하여 계산한다
    ///////////////////////////////////////////////////////
...
//        console.log("모바일:"+ mob);
    ...
    sld.on("dragstop touchend", function () {
        ...
        // 1. 슬라이드 left 이동 변경값체크
        sleft = $(this).offset().left;
        // offset().left 현재 선택요소의 left 값을 리턴
        //console.log("left:" + sleft);

        // 2. left값이 -110% 보다 작으면 왼쪽으로 이동하기 
        // -win*1.1 은 윈도우폭만큼 나가있는 px 크기의 110% 확장
        if (sleft < -win * 1.1) {

            //왼쪽이동 애니메이션 : -200% 이동과 동일하게~!
            $(this).stop().animate({
                    left: -win * 2 + "px"
                }, 1500, "easeOutCubic",
                function () { // 애니 후
                    // 이동 후에 앞에 두 개의 슬라이드가 있게되므로
                    // 하나를 뒤로 이동시킨 후 left 값을 다시 -100%로 변경
                    // 처음과 똑같은 상태로 만들어준다!
        ...
        // 3. left 값이 -90% 보다 크면 오른쪽으로 이동하기 
            ...
            $(this).stop().animate({
                ...
                function () { // 애니후
                    // 이동후에는 앞에 아무것도 없게되므로
                    // 맨 뒤 li 를 맨앞으로 이동후 left 값을 -100%변경
                    // 처음과 같은 상태로 만들어준다
        ...
        //4. 범위에 들지 않는 경우(-110% ~ -90%) 제자리로 돌아가기
        else {
    ...
    */ //////////////////////////////////////////////
    let goRight = function () {
        ...
        // 슬라이드 오른쪽이동하기 (드래그이동 if문과 동일) //

        // 광드래그 막기용 커버작동!
        prot.show();

        // 오른쪽이동 애니메이션 : 0%이동과 동일!
        sld.stop().animate({
                left: "0px"
            }, 1500, "easeOutCubic",
            function () { /// 애니후 ////
    ...
    */ ///////////////////////////////////////////////
    let banTit = function () {
        ...
        // 0. h2.btit요소 먼저 지우고 시작함!
        $(".btit").remove();

        // 배너 도착 후 타이틀을 슬라이드에 h2 태그로 넣기
        // 대상: .slide li
        // 실제배너는 맨앞에 하나있고 두번째이므로 1번이다~!
        sld.find("li").eq(1)
            .append('<h2 class="btit disableselect disableddrag"></h2>');
        // class명에 disableselect,disabledrag는
        // 드래그 시 글자가 영역 잡히는 것을 막기위한 css
        // 클래스 적용이다!

        // 2-0. 슬라이드 이밎에 따라 글자 위치 조정학
        // 2번과 3번 슬라이드만 오른쪽이고 나머지는 왼쪾임
        let tpos = "20%" // 왼쪽기준
