...
jQuery(document).ready(function ($) {
    ...
    $("#"+pm).addimagezoom({
        ...
        //갤러리관련 설정
        //갤러리 설정시 id명과 같은 이름의 class를 썸네일 박스에 주고 
        //thumbs라는 클래스명도 주면 하나의 갤러리 셋이 완성된다.
        
    }); /// multizoom 플러그인 ////////////////
    // 멀티줌 플러그인에서 m1 이미지셋만 크기가 700 * 700 이고
    // 다른이미지는 이것보다 작다. 플러그인은 원본 사이즈로
    // 축소/확대를 하므로 작은 이미지로 나오는 것을 고치려면 
    // 이미지를 700 * 700 으로 변경하여 넣으면 된다!
...
$(".chg_num img").click(function(){
    ...
    // 출력대상: #sum (input 요소니까 value 값으로 넣어준다 : text(X) val(O))
    $("#sum").val(gnum);
